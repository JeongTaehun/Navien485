...
# Home Assistant 애드온의 표준 경로인 /data/options.json 을 기본값으로 사용합니다.
def load_config(path="/data/options.json"):
    """
    지정된 경로에서 JSON 설정 파일을 읽어 전체 내용을 반환합니다.
    """
    try:
        with open(path, "r") as f:
            # 파일 전체를 읽어서 config 딕셔너리로 반환합니다.
            config = json.load(f)
        return config
        ...
        print(f"Error: 설정 파일({path})의 JSON 형식이 올바르지 않습니다.")
        return {}
...
# 2. 불러온 config에서 'MQTT'와 'TOPIC' 딕셔너리를 안전하게 가져옵니다.
#    .get(key, {})를 사용하면 해당 키가 없을 때 에러 대신 빈 딕셔너리를 반환하여 안전합니다.
MQTT_CONF = config.get("MQTT", {})
TOPIC_CONF = config.get("TOPIC", {})

# 3. 각 딕셔너리에서 최종 설정값들을 추출합니다.
#    마찬가지로 .get()을 사용하여 특정 값이 없더라도 기본값을 사용하도록 합니다.
MQTT_USERNAME = MQTT_CONF.get("username", "")
MQTT_PASSWORD = MQTT_CONF.get("password", "")
MQTT_SERVER   = MQTT_CONF.get("server")  # 필수 값이므로 기본값 없음
...
# 필수 값인 MQTT_SERVER가 설정되었는지 확인
if not MQTT_SERVER:
    print("치명적 오류: MQTT 서버 주소('server')가 설정되지 않았습니다. 애드온 구성을 확인하세요.")
else:
    # 결과 출력
    print("--- MQTT 설정 값 ---")
    ...
    print("\n--- TOPIC 설정 값 ---")
    print(f"ROOT_TOPIC_NAME: {ROOT_TOPIC_NAME}")
    print(f"HOMEASSISTANT_ROOT_TOPIC_NAME: {HOMEASSISTANT_ROOT_TOPIC_NAME}")
...
class Wallpad:
    ...
    def on_disconnect(self, client, userdata, rc, properties=None, *args, **kwargs):
        print(f"[WARN] Disconnected (rc={rc}), attempting to reconnect...")
        try:
            time.sleep(1)  # 잠시 대기 후 재연결 시도
            client.reconnect()
